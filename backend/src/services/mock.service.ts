...
const MOCK_QUESTIONS: Question[] = [
  { id: uuid(), text: "Define the term 'photosynthesis' and explain its importance in the ecosystem.", marks: 5, order: 1, displayNumber: "1" },
  { id: uuid(), text: "State two differences between mitosis and meiosis.", marks: 4, order: 2, displayNumber: "2" },
  { id: uuid(), text: "What is Newton's Second Law of Motion? Provide the mathematical formula.", marks: 3, order: 3, displayNumber: "3" },
  { id: uuid(), text: "Explain the water cycle with the help of a diagram.", marks: 6, order: 4, displayNumber: "4" },
  { id: uuid(), text: "Solve the following quadratic equation: x² - 5x + 6 = 0", marks: 4, order: 5, displayNumber: "5" },
  { id: uuid(), text: "Describe the structure of an atom.", marks: 3, order: 6, displayNumber: "6" },
  { id: uuid(), text: "What is the chemical formula for glucose? Describe its role in cellular respiration.", marks: 5, order: 7, displayNumber: "7" },
  { id: uuid(), text: "Write a short note on the French Revolution.", marks: 6, order: 8, displayNumber: "8" },
  { id: uuid(), text: "Explain the concept of supply and demand with a graph.", marks: 5, order: 9, displayNumber: "9" },
  { id: uuid(), text: "Define acceleration and give its SI unit.", marks: 2, order: 10, displayNumber: "10" },
  { id: uuid(), text: "Explain the greenhouse effect.", marks: 4, order: 11, parentNumber: "11", subQuestion: "a", displayNumber: "11(a)" },
  { id: uuid(), text: "List three greenhouse gases and their sources.", marks: 3, order: 12, parentNumber: "11", subQuestion: "b", displayNumber: "11(b)" },
];

function createMockAnswers(questions: Question[]): Answer[] {
...
function createMockMappings(
  ...
  return questions.map((q) => {
    ...
    if (!answer) {
      return { questionId: q.id, status: "unanswered" as const };
    }
...
export class MockExtractionService implements ExtractionService {
  async extractQuestions(_filePath: string): Promise<Question[]> {
    await delay(1500);
    return MOCK_QUESTIONS.map((q) => ({ ...q, id: uuid() }));
  }

  async extractAnswers(_filePath: string): Promise<Answer[]> {
    await delay(2000);
  ...
  async fullExtraction(
    _questionPaperPath: string,
    _answerSheetPath: string,
    ...
    await delay(2000);
    const questions = MOCK_QUESTIONS.map((q) => ({ ...q, id: uuid() }));
